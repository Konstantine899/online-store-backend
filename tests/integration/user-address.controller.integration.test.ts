...
describe('UserAddressController (integration)', () => {
    ...
    it('GET /user/addresses should return 401 without auth', async () => {
        await request(app.getHttpServer())
            .get('/user/addresses')
            .expect(HttpStatus.UNAUTHORIZED);
    });

    it('POST /user/addresses should return 401 without auth', async () => {
        await request(app.getHttpServer())
            .post('/user/addresses')
    ...
    it('CRUD flow with auth', async () => {
        ...
        // Get list
        const listRes = await http.get('/user/addresses').set('Authorization', `Bearer ${token}`).expect(HttpStatus.OK);
        const list = listRes.body.data || listRes.body;
        expect(Array.isArray(list)).toBe(true);

        // Get one
        const oneRes = await http.get(`/user/addresses/${id}`).set('Authorization', `Bearer ${token}`).expect(HttpStatus.OK);
        ...
        // Remove
        await http.delete(`/user/addresses/${id}`).set('Authorization', `Bearer ${token}`).expect(HttpStatus.OK);
    });

    it('validation errors on create (400)', async () => {
        const token = await authLoginAs(app, 'user');
        const http = request(app.getHttpServer());
    ...
    it('404 on get/update/remove non-existent address', async () => {
        const token = await authLoginAs(app, 'user');
        const http = request(app.getHttpServer());

        const nonexistentId = 999999;
        await http.get(`/user/addresses/${nonexistentId}`).set('Authorization', `Bearer ${token}`).expect(HttpStatus.NOT_FOUND);
        await http
            .put(`/user/addresses/${nonexistentId}`)
            .set('Authorization', `Bearer ${token}`)
            .send({ title: 'X' })
            .expect(HttpStatus.NOT_FOUND);
        await http.delete(`/user/addresses/${nonexistentId}`).set('Authorization', `Bearer ${token}`).expect(HttpStatus.NOT_FOUND);
    });

    it('400 on invalid id param (not a number)', async () => {
        const token = await authLoginAs(app, 'user');
        const http = request(app.getHttpServer());
        await http.get('/user/addresses/not-a-number').set('Authorization', `Bearer ${token}`).expect(HttpStatus.BAD_REQUEST);
        await http.put('/user/addresses/not-a-number').set('Authorization', `Bearer ${token}`).send({ title: 'X' }).expect(HttpStatus.BAD_REQUEST);
        await http.delete('/user/addresses/not-a-number').set('Authorization', `Bearer ${token}`).expect(HttpStatus.BAD_REQUEST);
    });

    it('setting new default unsets previous default (order and flags)', async () => {
        const token = await authLoginAs(app, 'user');
        const http = request(app.getHttpServer());
        ...
        // Cleanup
        await http.delete(`/user/addresses/${id1}`).set('Authorization', `Bearer ${token}`).expect(HttpStatus.OK);
        await http.delete(`/user/addresses/${id2}`).set('Authorization', `Bearer ${token}`).expect(HttpStatus.OK);
    });
});
