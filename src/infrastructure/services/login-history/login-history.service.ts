...
export class LoginHistoryService implements ILoginHistoryService {
    ...
    async logSuccessfulLogin(
        ...
        try {
            ...
            this.logger.log(
                `Successful login logged for user ${userId} from IP ${ipAddress}`,
            );
            return loginRecord;
        } catch (error) {
            this.logger.error(
                `Failed to log successful login for user ${userId}:`,
                error,
            );
            throw error;
        }
    ...
    async logFailedLogin(
        ...
        try {
            ...
            this.logger.warn(
                `Failed login logged for user ${userId} from IP ${ipAddress}: ${failureReason}`,
            );
            return loginRecord;
        } catch (error) {
            this.logger.error(
    ...
    async getUserLoginHistory(
        ...
        return this.loginHistoryRepository.findUserLoginHistory(
            userId,
            limit,
            offset,
        );
    ...
    async getRecentLoginsByIp(
        ipAddress: string,
        hours = 24,
    ): Promise<LoginHistoryModel[]> {
        return this.loginHistoryRepository.findRecentLoginsByIp(
            ipAddress,
            hours,
        );
    }

    async getFailedLoginsByUser(
        userId: number,
        hours = 24,
    ): Promise<LoginHistoryModel[]> {
        return this.loginHistoryRepository.findFailedLoginsByUser(
            userId,
            hours,
        );
    }
    ...
    async getUserLoginStats(userId: number): Promise<{
        ...
        const [totalLogins, successfulLogins, failedLogins, recentLogins] =
            await Promise.all([
        ...
        return {
            ...
            failedLogins,
            lastLoginAt:
                recentLogins.length > 0 ? recentLogins[0].loginAt : null,
        };
    }
    ...
    async cleanupOldLoginHistory(daysToKeep = 90): Promise<number> {
        try {
            const deletedCount =
                await this.loginHistoryRepository.deleteOldLoginHistory(
                    daysToKeep,
                );
            this.logger.log(
                `Cleaned up ${deletedCount} old login history records`,
            );
            return deletedCount;
        } catch (error) {
