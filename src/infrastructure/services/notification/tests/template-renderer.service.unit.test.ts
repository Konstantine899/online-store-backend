...
describe('TemplateRendererService', () => {
    ...
    const createTemplateVariables = (overrides: Partial<TemplateVariables> = {}): TemplateVariables => ({
        name: 'John',
        orderNumber: '12345',
        userName: 'John',
        greeting: 'Hello',
    ...
    describe('renderTemplate', () => {
        it('should render template with variables successfully', async () => {
            const template = createTemplate('Hello {{name}}, your order {{orderNumber}} is ready!');
        ...
        it('should render template with repeated variables', async () => {
            const template = createTemplate('{{greeting}} {{name}}, {{greeting}} again!');
            const variables = createTemplateVariables({ 
                greeting: 'Hello', 
                name: 'Bob' 
            });
        ...
        it('should render template with different data types', async () => {
            const template = createTemplate('Number: {{count}}, Boolean: {{active}}, Object: {{data}}');
            const variables = createTemplateVariables({
                count: 42,
                active: true,
                data: { key: 'value' },
        ...
        it('should fail with multiple missing variables', async () => {
            const template = createTemplate('Hello {{name}}, your order {{orderNumber}} from {{store}} is ready!');
            const variables = {
                name: 'John' // Передаем только name
            ...
            expect(result.success).toBe(false);
            expect(result.error).toBe('Отсутствуют переменные: orderNumber, store');
            expect(result.missingVariables).toEqual(['orderNumber', 'store']);
        });

        it('should handle template with no variables', async () => {
            const template = createTemplate('This is a static template with no variables.');
            const variables = createTemplateVariables({});
    ...
    describe('Edge cases', () => {
        ...
        it('should handle variables with special characters', async () => {
            const template = createTemplate('Hello {{user-name}}, your order {{order_number}} is ready!');
            const variables = createTemplateVariables({
                'user-name': 'John Doe',
                order_number: '12345',
            });
    ...
    describe('Performance Tests', () => {
        ...
        it('should handle bulk template rendering efficiently', async () => {
            ...
            const results = await Promise.all(
                templates.map(template => service.renderTemplate(template, variables))
            );
    ...
    describe('Edge Cases and Stress Tests', () => {
        it('should handle very long templates efficiently', async () => {
            ...
            const startTime = Date.now();
            const result = await service.renderTemplate(longTemplate, variables);
            const endTime = Date.now();
        ...
        it('should handle templates with many variables efficiently', async () => {
            ...
            const startTime = Date.now();
            const result = await service.renderTemplate(manyVariablesTemplate, manyVariables);
            const endTime = Date.now();
