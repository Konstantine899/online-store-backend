...
describe('CartService', () => {
    ...
    describe('getCart', () => {
        it('должен получить корзину, создать новую или пробросить ошибку', async () => {
            ...
            result = await service.getCart(
                requestWithoutCart as Request,
                mockResponse as Response,
            );
            expectCartResponse(result, []);
    ...
    describe('appendToCart', () => {
        it('должен добавить товар, создать корзину или выбросить ошибки', async () => {
            ...
            let result = await service.appendToCart(
                mockRequest as Request,
                mockResponse as Response,
                1,
                2,
            ...
            result = await service.appendToCart(
                requestWithoutCart as Request,
                mockResponse as Response,
                1,
                2,
            ...
            productRepository.fidProductByPkId.mockResolvedValue(
                null as unknown as ProductModel,
            );
    ...
    describe('Cart operations', () => {
        const testOperations = [
            {
                name: 'increment',
                method: 'increment',
                expectedCall: 'increment',
            },
            {
                name: 'decrement',
                method: 'decrement',
                expectedCall: 'decrement',
            },
            {
                name: 'removeProductFromCart',
                method: 'removeProductFromCart',
                expectedCall: 'removeFromCart',
            },
        ];
        ...
        testOperations.forEach(({ name, method, expectedCall }) => {
            it(`должен выполнить операцию ${name}`, async () => {
                ...
                setupMockProduct();
                setupMockCart([createMockCartProduct(mockProduct)]);
                (cartRepository as unknown as Record<string, jest.Mock>)[
                    expectedCall
            ...
            it(`должен обработать ${name}`, () => {
                const cartProducts = products.map((p) =>
                    createMockCartProduct(
                        createMockProduct(p.id, p.name, p.price),
                        p.quantity,
                    ),
                );
                const cart = createMockCart(cartProducts);
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const transformData = (service as any).TransformData.bind(
                    service,
                );
    ...
    describe('Integration scenarios', () => {
        it('должен выполнить полный цикл работы с корзиной', async () => {
            ...
            result = await service.clearCart(
                mockRequest as Request,
                mockResponse as Response,
            );
            expect(result.products).toEqual([]);
